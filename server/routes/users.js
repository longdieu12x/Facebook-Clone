...
router.get("/:id", async (req, res) => {
	try {
		...
		const { password, updatedAt, ...other } = user._doc; //._doc carry all object
...
router.put("/:id/unfollow", async (req, res) => {
	if (req.body.user_id !== req.params.id) {
		try {
			const user = await User.findById(req.params.id); //user can tim
			const currentUser = await User.findById(req.body.user_id); // user dang nhap
			if (user.followers.includes(req.body.user_id)) {
